...
public class WarmapRunner {
    ...
    public static void main(String[] args) {
         ...
         */
        Map<Author, List<Picture>> mapa1 = new HashMap<>();

        for (Author a : authors) {
            List<Picture> listPicture = new ArrayList<>();
            mapa1.put(a, listPicture);
            for (Picture p : pictures) {
                if (a.getId()== p.getAuthorId()) {
                    listPicture.add(p);
        ...
        for (Map.Entry<Author, List<Picture>> entry : mapa1.entrySet()) {
            int numberOfPictures=entry.getValue().size();

            System.out.println(entry);

        }
        Map<Team, Map<Author,List<Picture>>> mapTeam = new HashMap<>();
        for( Team t: teams){
            Map<Author,List<Picture>>mapPom= new HashMap<>();
            mapTeam.put(t,mapPom);
            for (Map.Entry<Author, List<Picture>> entry : mapa1.entrySet()) {
                if(t.getId()==entry.getKey().getId()){
                    mapTeam.put(t,entry.getKey().getId());
                }
            }
